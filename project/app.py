...
def insert_ec():
    ...
    Ecopetrol = requests.get(
        'https://financialmodelingprep.com/api/v3/quote-short/EC?apikey=2c18aef9c283317435d76672c51df35d')
...
def insert_ibm():
    print('IBM')
    IBM = requests.get(
        'https://financialmodelingprep.com/api/v3/quote-short/IBM?apikey=2c18aef9c283317435d76672c51df35d')
    IBM = IBM.json()
...
def insert_btc():
    print('Bitcoin')
    BTC = requests.get(
        'https://financialmodelingprep.com/api/v3/quote-short/BTC-USD?apikey=2c18aef9c283317435d76672c51df35d')
    BTC = BTC.json()
...
def insert_eth():
    print('Ethereum')
    ETH = requests.get(
        'https://financialmodelingprep.com/api/v3/quote-short/ETH-USD?apikey=2c18aef9c283317435d76672c51df35d')
    ETH = ETH.json()
...
def insert_ada():
    print('Cardano')
    ADA = requests.get(
        'https://financialmodelingprep.com/api/v3/quote-short/ADA-USD?apikey=2c18aef9c283317435d76672c51df35d')
    ADA = ADA.json()
...
# 300 = 5 minutes
def countdown(countDownTime=120):
    global my_timer
...
def hello_world():
    ...
    insert_ada()
    dava_chart=get_dava()
    x=list(dava_chart['date'].values)
    print(x)
    y=list(dava_chart['price'].values)
    print(y)
...
def index(currency='dava'):
    ...
    if currency == 'ec':
        ...
        if (ec.find()!=-1):
            ec_chart=get_ec()
            x=list(ec_chart['date'].values)
            y=list(ec_chart['price'].values)
        else:
            x=0
            y=0
    ...
    if currency == 'ibm':
        ...
        if (ibm.find()!=-1):
            ibm_chart=get_ibm()
            x=list(ibm_chart['date'].values)
            y=list(ibm_chart['price'].values)
        else:
            x=0
            y=0
    ...
    if currency == 'btc':
        ...
        if (btc.find()!=-1):
            btc_chart=get_btc()
            x=list(btc_chart['date'].values)
            y=list(btc_chart['price'].values)
        else:
            x=0
            y=0
    ...
    if currency == 'eth':
        ...
        if (eth.find()!=-1):
            eth_chart=get_eth()
            x=list(eth_chart['date'].values)
            y=list(eth_chart['price'].values)
        else:
            x=0
            y=0
    ...
    if currency == 'ada':
        ...
        if (ada.find()!=-1):
            ada_chart=get_ada()
            x=list(ada_chart['date'].values)
            y=list(ada_chart['price'].values)
        else:
            x=0
            y=0
